...
object		Euler11
{
	...
	val grid =
		A(	A(  8,  2, 22, 97, 38, 15,  0, 40,  0, 75,  4,  5,  7, 78, 52, 12, 50, 77, 91,  8 ),
	...
	def
	contig4(x: Int, y: Int): Seq[Seq[(Int, Int)]] =
		(	// Left-to-right
			if (x + 3 < gridSize)
				List(for (c <- x to x + 3) yield (c, y))
			else
				Nil
		) ++ (
			// Top-to-bottom
			if (y + 3 < gridSize)
				List(for (r <- y to y + 3) yield (x, r))
			else
				Nil
		) ++ (
	   		// Top-left-to-bottom-right
			if (x + 3 < gridSize && y + 3 < gridSize)
				List(for (d <- 0 to 3) yield (x + d, y + d))
			else
				Nil
		) ++ (
			// Top-right-to-bottom-left
			if (x - 3 >= 0 && y + 3 < gridSize)
				List(for (d <- 3 to 0 by -1) yield (x - d, y + d))
			else
				Nil
		)
	...
	(	for {
		...
		yield
			(coords map { (coord: (Int, Int)) => grid(coord._1)(coord._2) }) product
	)	max

	def
	main(args: Array[String]): Unit =
		printf("Euler11: %d%n", euler11)
}
